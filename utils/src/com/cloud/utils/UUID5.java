 ...
 *
 * @author Luther Tychonievich. Released into the public domain.
 */
public class UUID5 {
    /**
     * The following namespace is a type-5 UUID of "cloudstack" in the null namespace
     */
    public static final UUID CLOUDSTACK_NAMESPACE = UUID.fromString("b960ae48-c19c-5ce4-9e90-4a4014c35eb0");
    ...
    /**
     * Similar to UUID.nameUUIDFromBytes, but does version 5 (sha-1) not version
     * 3 (md5). Using the Cloudstack namespace.
     *
     * @param name
     *            The bytes to use as the "name" of this hash
     * @return the UUID object
     */
    public static UUID fromBytesACS(byte[] name) {
        if (name == null) {
            throw new NullPointerException("name == null");
        }
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-1");
            md.update(asBytes(CLOUDSTACK_NAMESPACE.getMostSignificantBits(), ByteOrder.BIG_ENDIAN));
            md.update(asBytes(CLOUDSTACK_NAMESPACE.getLeastSignificantBits(), ByteOrder.BIG_ENDIAN));
     ...
     * @return the UUID object
     */
    public static UUID fromUTF8ACS(String name) {
        return UUID5.fromBytes(CLOUDSTACK_NAMESPACE, name.getBytes(Charset.forName("UTF-8")));
    }
     ...
     *
     * @param bytes
     *          The UUID in byte format, as generated by toByteArray().
